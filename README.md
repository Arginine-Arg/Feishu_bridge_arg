...
Version 1.5.2 fixes legacy-pane adoption: Bridge loads and validates the old thread before creating a remote pane, discovers the pane's actual `CODEX_HOME` across its process tree, and cleans up a newly created pane if validation fails. It retains automatic legacy-pane adoption for the opt-in structured API backend. The existing terminal backend remains the default. See [configuration, validated capabilities, and limitations](./docs/structured-backend.md) before enabling it. Updating does not migrate existing terminal sessions automatically.
...
- **Readable long answers**: terminal activity is compacted into a collapsed summary; completed code and diffs use expandable panels, while tables and ASCII diagrams keep monospace alignment across message chunks.
...
sh /tmp/install-arg-bridge.sh --version 1.5.2
...
`restart` restarts only the bridge relay. On Linux it first refreshes the systemd unit and then reconnects; bridge-managed tmux/Codex/Claude processes are not terminated with the service. A running task continues in tmux, its already-detached intermediate output is not replayed, and the next Feishu message attaches to the same native session for that profile and chat/topic scope.
...
| `/reconnect` | Reconnect WebSocket; preserve a running tmux task by default, or wait for it with `--wait` |
...
- **Low-emphasis terminal activity**: Codex `Ran`/`Explored` frames, Claude tool surfaces, and non-interactive command echoes are retained inside one collapsed **Execution activity** panel. Text-mode streams show only a compact item count, and final answer messages omit the trace entirely. Normal progress notes, code, tables, and the final answer remain complete; completed fenced code and diffs use expandable panels, while tables and terminal diagrams keep monospace alignment. Native pickers such as `/model` are never compacted, so every option stays available to the signed control card.
...
- **Persistent tmux identity**: a scope records its managed tmux session and currently adopted agent pane. Closing a local terminal, detaching tmux, `arg-bridge restart`, service auto-restart, or `/reconnect` only detaches the bridge relay: it never writes Ctrl-C to the tmux agent and never creates a new native conversation. Reconnection strictly validates profile, agent kind, Feishu chat/topic scope, and workspace, so it cannot adopt another conversation's pane; the next Feishu turn reattaches to the original session. If a restart occurs while a task is running, the task continues in tmux, while the disconnected bridge deliberately does not replay or post that turn's intermediate output. If you manually run `codex resume` or `claude --resume` in a new selected pane of that managed session, the bridge adopts and persists that pane; later Feishu input and `/tmux tail` target the resumed conversation. A real reboot of the host that runs tmux necessarily ends its processes; move the bridge and tmux to the durable server host for work that must survive client shutdowns.
...
- **Message reply mode**: `message card` streams the final reply; `plain text` sends once after the run finishes. Long structured answers automatically use cards so code fences are not split or flattened into ordinary prose.
