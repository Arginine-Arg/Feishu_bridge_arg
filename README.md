...
sh /tmp/install-arg-bridge.sh --version 0.6.63
...
- **Low-emphasis terminal activity**: Codex `Ran`/`Explored` frames, Claude tool surfaces, and non-interactive command echoes are retained inside one collapsed **Execution activity** panel (or a markdown quote in text mode). Normal progress notes and the final answer remain prominent. Native pickers such as `/model` are never compacted, so every option stays available to the signed control card.
