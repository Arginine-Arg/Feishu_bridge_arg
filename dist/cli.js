...
var package_default = {
  ...
  repository: {
    type: "git",
    url: "git+https://github.com/Arginine-Arg/Feishu_bridge_arg.git"
  },
  bugs: {
    url: "https://github.com/Arginine-Arg/Feishu_bridge_arg/issues"
  },
  homepage: "https://github.com/Arginine-Arg/Feishu_bridge_arg#readme",
  bin: {
    "arg-bridge": "./bin/arg-bridge.mjs",
    "lark-channel-bridge": "./bin/arg-bridge.mjs"
  },
  exports: {
    ".": {
...
async function runKillCli(target) {
  if (!target) {
    console.error("\u7528\u6CD5: arg-bridge kill <bot id \u6216\u5E8F\u53F7>");
    process.exit(1);
  }
  const entry = resolveTarget(target);
  if (!entry) {
    console.error(`\u2717 \u6CA1\u627E\u5230\u5339\u914D\u7684 bot:${target}`);
    console.error("  \u7528 `arg-bridge ps` \u770B\u53EF\u9009\u76EE\u6807\u3002");
    process.exit(1);
  }
...
rem Auto-generated by arg-bridge. Do not edit.\r
...
# Auto-generated by arg-bridge. Do not edit.
...
async function runSecretsSet(appId, opts = {}) {
  if (!appId) {
    console.error("\u7528\u6CD5: arg-bridge secrets set --app-id <id>");
    process.exit(1);
  }
...
async function runSecretsRemove(appId, opts = {}) {
  if (!appId) {
    console.error("\u7528\u6CD5: arg-bridge secrets remove --app-id <id>");
    process.exit(1);
  }
...
async function resolveBootstrapAppConfig(opts) {
  if (!opts.appId) {
    if (!isInteractiveTerminal()) {
      throw new Error(
        "\u5F53\u524D\u6CA1\u6709\u914D\u7F6E\uFF0C\u975E\u4EA4\u4E92\u6A21\u5F0F\u65E0\u6CD5\u5B8C\u6210\u626B\u7801\u521B\u5EFA\u5E94\u7528\u3002\u8BF7\u5148\u5728\u7EC8\u7AEF\u8FD0\u884C `arg-bridge run` \u5B8C\u6210\u9996\u6B21\u521D\u59CB\u5316\uFF0C\u6216\u4F20\u5165 --app-id \u548C --app-secret\u3002"
      );
    }
...
var SERVICE_NAME = "arg-bridge.bot";
...
function windowsTaskName(profile2 = paths.profile) {
  return `ArgBridge.Bot.${serviceProfileId(profile2)}`;
}
...
function restartInstruction(profile2) {
  ...
  return `Restart the current profile: arg-bridge restart${suffix}; for foreground runs, press Ctrl+C and rerun arg-bridge run${suffix}.`;
}
...
async function assertLockNotHeldByAnotherRuntime(kind, target, adapter, opts = {}) {
  for (; ; ) {
    ...
    if (!opts.confirmStopRuntimeLockProcess && (!process.stdin.isTTY || !process.stdout.isTTY)) {
      console.error(
        `  \u975E\u4EA4\u4E92\u6A21\u5F0F\u65E0\u6CD5\u786E\u8BA4\u505C\u6B62 ${kind === "profile" ? "profile" : "app"} \u5360\u7528\u8FDB\u7A0B\u3002\u8BF7\u5148\u7528 \`arg-bridge ps\` \u67E5\u770B\u5E76\u7528 \`arg-bridge kill <bot id>\` \u505C\u6B62\u540E\u91CD\u8BD5\u3002`
      );
      process.exit(1);
    }
...
function isLiveCommandResultOutput(text, prompt) {
  ...
  if (command === "/skills") {
    return /\bchoose an action\b/i.test(trimmed) || /\bavailable skills\b/i.test(trimmed) || /(?:^|\n)\s*(?:[›>▸*+-]\s*)?\d{1,2}[.)、:\s-]+\S/u.test(trimmed) && /\b(?:skills?|enable|disable|action)\b/i.test(trimmed);
  }
  return true;
}
function isModelPickerOutput(text) {
  return /\bselect model and effort\b/i.test(text) || /\bselect\s+(?:a\s+)?model\b/i.test(text) || /\breasoning effort\b/i.test(text) || /press\s+enter\s+to\s+confirm/i.test(text) || /(?:^|\n)\s*(?:[›>▸*+-]\s*)?\d{1,2}[.)、:\s-]+(?:gpt-|low|medium|high)\S*/iu.test(text);
}
...
function isCodexStatusPanelLine(trimmed) {
  if (!trimmed.startsWith("\u2502")) return false;
  return /(?:>_\s*)?OpenAI Codex/i.test(trimmed) || /\b(?:Model|Model provider|Directory|Permissions|Agents\.md|Account|Collaboration mode|Session|Token usage|Limits):/i.test(trimmed) || /chatgpt\.com\/codex\/settings\/usage/i.test(trimmed) || /information on rate limits and credits/i.test(trimmed) || /^│\s*│?$/.test(trimmed);
}
...
function isStaleStatusSnapshotForPrompt(text, prompt) {
  ...
  const compact = compactForNoiseMatch(text);
  return compact.includes("openaicodex") && compact.includes("model:") && compact.includes("directory:") && compact.includes("tokenusage:");
}
function isStaleGoalUsageSnapshotForPrompt(text, prompt) {
  const command = prompt.trim().toLowerCase();
...
function resolveModelArg(agentKind, value) {
  ...
  return normalized === DEFAULT_MODEL ? void 0 : normalized;
}
function modelLabel(agentKind, value) {
  const normalized = normalizeModelSelection(agentKind, value);
...
\u7528\u6CD5\uFF1A\`/session live\` \u6216 \`/session turn\`\uFF1B\u5B8C\u6574\u8FD0\u884C\u72B6\u6001\u7528 \`/status\`\u3002`
  ...
  const next = action === "live" || action === "on" ? "live" : action === "turn" || action === "off" ? "turn" : void 0;
  if (!next) {
...
// src/card/interactive-prompt.ts
var BRIDGE_PROMPT_CALLBACK_MARKER = "__bridge_prompt";
var PROMPT_CALLBACK_ACTION = "prompt_answer";
...
function renderAskCard(questions, sign2) {
  const elements = [];
...
function renderPlanCard(plan, sign2) {
  ...
  return {
    ...
    body: {
      elements: [
        { tag: "markdown", content: "**\u{1F4DD} \u6267\u884C\u8BA1\u5212\uFF08\u5F85\u786E\u8BA4\uFF09**" },
...
function buildPromptCard(toolName, input, sign2) {
  if (toolName === "AskUserQuestion") {
    const questions = parseQuestions(input);
    return questions.length > 0 ? renderAskCard(questions, sign2) : void 0;
  }
  if (toolName === "ExitPlanMode") {
    const plan = asString(input?.plan);
    return plan.trim() ? renderPlanCard(plan, sign2) : void 0;
  }
  return void 0;
}
async function consumeInteractivePrompts(events, deps) {
  const seen = /* @__PURE__ */ new Set();
  try {
...
function verifyPromptToken(deps, payload, scope, operatorId) {
  const token = typeof payload.bridge_token === "string" ? payload.bridge_token : "";
...
function matchesExpected(payload, expected) {
  return (expected.runId === void 0 || payload.r === expected.runId) && payload.s === expected.scope && payload.c === expected.chatId && payload.o === expected.operatorOpenId && payload.a === expected.action && (expected.policyFingerprint === void 0 || payload.fp === expected.policyFingerprint);
}
...
function isBridgeSelfReply(channel, evt) {
  ...
  return record.bridge === true || record.bridgeReply === true || record.source === "arg-bridge";
}
...
function mapSenderType(raw) {
  if (raw === "user") return "user";
  if (raw === "app" || raw === "bot") return "bot";
  return void 0;
}
async function normalizeItemToQuoted(channel, parent, fetchSubMessages) {
  if (!parent.message_id) return void 0;
...
async function fetchTopicContext(channel, threadId, opts) {
  const collected = [];
  let pageToken;
  try {
    do {
  ...
  const exclude = opts.excludeIds ?? /* @__PURE__ */ new Set();
  const relevant = collected.filter(
    (m) => m.message_id && !exclude.has(m.message_id) && !m.deleted
...
async function safeFetchRaw(channel, messageId) {
  try {
...
async function runAgentBatch(deps) {
  ...
  if (mode === "topic" && threadId && !sessions.getRaw(scope)) {
    const exclude = /* @__PURE__ */ new Set([...batchIds, ...quoteTargets]);
...
\`\`\`
${escapeFence2(interaction.prompt)}
\`\`\``
        },
        ...buttons
      ]
...
function toPromptTopicMessage(q) {
  return {
    messageId: q.messageId,
    senderId: q.senderId,
    ...q.senderName ? { senderName: q.senderName } : {},
