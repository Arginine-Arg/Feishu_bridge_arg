...
sh /tmp/install-arg-bridge.sh --version 0.6.8
# 无权写入 npm 默认全局目录时：
...
npm install -g --ignore-scripts --install-links=true \
  "git+https://github.com/Arginine-Arg/Feishu_bridge_arg.git#v0.6.8"
```

`--install-links=true` 防止 npm 11 把全局包保留为临时 Git clone 的软链；`--ignore-scripts` 避免依赖 lifecycle 出现 `spawn /bin/sh ENOENT`，arg-bridge 运行时不依赖这些依赖包的 postinstall。只能走 SSH 时，保留相同参数并使用 `git+ssh://git@github.com/Arginine-Arg/Feishu_bridge_arg.git#v0.6.8`。
