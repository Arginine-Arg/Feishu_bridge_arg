...
正式版 `1.5.2` 修复 legacy pane 迁移：Bridge 会先加载并校验旧 thread，再创建 remote pane；同时从 pane 进程树读取真实 `CODEX_HOME`，迁移失败时清理本次新建的 pane。默认仍使用现有终端后端。启用前请阅读[配置方式、已验证能力与限制](./docs/structured-backend.md)。升级不会自动迁移旧终端会话。
...
`--install-links=true` 防止 npm 11 把全局包保留为临时 Git clone 的软链；`--ignore-scripts` 避免依赖 lifecycle 出现 `spawn /bin/sh ENOENT`，arg-bridge 运行时不依赖这些依赖包的 postinstall。只能走 SSH 时，保留相同参数并使用 `git+ssh://git@github.com/Arginine-Arg/Feishu_bridge_arg.git#v1.5.2`。
